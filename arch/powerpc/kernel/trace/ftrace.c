...
static unsigned long find_ftrace_tramp(unsigned long ip, unsigned long target)
{
	...
	hash_for_each_possible(ppc_ftrace_stubs, stub, hentry, target)
		if (stub->target == target && is_offset_in_branch_range(stub->addr - ip))
			return stub->addr;
...
static int setup_mcount_compiler_tramp(unsigned long tramp)
{
	...
	if (patch_branch((u32 *)tramp, ftrace_target, 0)) {
...
__ftrace_modify_call_kernel(struct dyn_ftrace *rec, unsigned long old_addr, unsigned long addr)
{
	...
	/* Make sure that this is still a 24bit jump */
	if (!is_bl_op(op)) {
		pr_err("Not expected bl: opcode is %lu\n", ppc_inst_as_ulong(op));
		return -EINVAL;
	}
...
{
	unsigned long target = ppc_global_function_entry((void *)FTRACE_REGS_ADDR);
	unsigned long stub_addr = (unsigned long)ftrace_tramp_init;
	struct ppc_ftrace_stub_data *stub;

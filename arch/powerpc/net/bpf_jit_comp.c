...
void bpf_jit_build_fentry_stubs(u32 *image, struct codegen_context *ctx)
{
	...
	// TODO: WARN_ON(!is_offset_in_branch_range(4 - (long)ctx->idx * 4));
					 ...
					 void *func_addr)
{
	...
	/* TODO: We only support functions with in-register arguments - upto 8 per powerpc ABI */
	if (nr_regs > 8)
		return -ENOTSUPP;
	...
	if (flags & BPF_TRAMP_F_CALL_ORIG) {
		/*
		 * The address in LR save area points to the correct point in the orignal function
		       ...
		       void *old_addr, void *new_addr)
{
	...
	if (!__bpf_address_lookup(bpf_func, &size, &offset, name)) {
		pr_err("bpf_arch_text_poke (0x%lx): kernel/modules are not supported\n", bpf_func);
		return -ENOTSUPP;
	}
	...
	if (offset) {
		if (poke_type != BPF_MOD_JUMP) {
			pr_err("bpf_arch_text_poke (0x%lx): calls are not supported in bpf prog body\n",
			       bpf_func);
			return -ENOTSUPP;
		}
	...
	if (!is_offset_in_branch_range((long)ip - 4 - bpf_func)) {
		pr_err("bpf_arch_text_poke (0x%lx): bpf prog too large, ool stub out of branch range\n",
		       bpf_func);
		return -ERANGE;
	}
